RSpec.describe InfernoTemplate::PatientGroup do
  let(:suite) { Inferno::Repositories::TestSuites.new.find('fhir_genomics_testing_test_suite') }
  let(:group) { suite.groups[1] }
  let(:session_data_repo) { Inferno::Repositories::SessionData.new }
  let(:test_session) { repo_create(:test_session, test_suite_id: 'fhir_genomics_testing_test_suite') }
